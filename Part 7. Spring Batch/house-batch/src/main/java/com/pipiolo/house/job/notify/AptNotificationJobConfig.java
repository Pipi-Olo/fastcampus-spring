...
import com.pipiolo.house.adapter.RealSendService;
...
import com.pipiolo.house.core.repository.AptNotificationRepository;
import com.pipiolo.house.core.repository.LawdRepository;
import com.pipiolo.house.core.service.AptDealService;
import com.pipiolo.house.job.validator.DealDateParameterValidator;
...
public class AptNotificationJobConfig {
    ...
    public ItemWriter<NotificationDto> aptNotificationWriter(
            ...
            RealSendService realSendService
    ) {
        return items -> {
//            items.forEach(item -> fakeSendService.send(item.getEmail(), item.toMessage()));
            items.forEach(item -> realSendService.send(item.getEmail(), item.toMessage()));
        };
    }
}
